            ...
            <div className="mx-auto max-w-[1400px] px-6">
                ...
                <div className="flex flex-col gap-32 lg:gap-56">
                    {showcaseData.items.map((item: any, index: number) => (
                        <div
                            key={item.number}
                            className={`flex flex-col lg:items-center gap-16 lg:gap-24 ${item.imagePos === 'left' ? 'lg:flex-row-reverse' : 'lg:flex-row'
                                }`}
                        >
                            {/* Text part */}
                            <Reveal delay={index * 0.1} className="flex-1 space-y-6">
